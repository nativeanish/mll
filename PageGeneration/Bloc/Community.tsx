...
function DiscordSvg({ size = 24 }: { size?: number }) {
  return (
    <svg
...
function RedditSvg({ size = 24 }: { size?: number }) {
  return (
    <svg
      viewBox="0 0 512 512"
      xmlns="http://www.w3.org/2000/svg"
...
function Community({ blockData }: { blockData: BlockData }) {
  const { url, description, title } = getStringFields(blockData.data, [
    "url",
    "description",
    "title",
  ]);
  const comtype = blockData.alt;
  return (
    <>
